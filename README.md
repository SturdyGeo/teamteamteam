...
- Compact rounded top navigation capsule is center-aligned and content-width (middle-top only) with org/project selectors and profile menu
- Project labels in nav selector now show project names only (no prefix code)
- Project dropdown allows checking multiple boards to display in a stacked view under the main board
- Stacked secondary boards can be reordered via left-side drag handles
